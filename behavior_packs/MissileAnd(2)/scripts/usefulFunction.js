...
export function isBlockUnder(dimension,location,under){
    const Target = {
        x:location.x,
        y:location.y-under,
        z:location.z
    }
    //world.sendMessage(`${dimension.getBlock(Target).typeId}`)
    if( dimension.getBlock(Target).typeId == `minecraft:air` ){
        return 1
    }
    else{
        return 0;
...
export function isBlockFront(dimension,location,player){
    let a = ``
    ...
    //world.sendMessage(`${dimension.getBlock(Target).typeId}`)
    a = dimension.getBlock(Target).typeId;
    if( dimension.getBlock(Target).typeId.includes(`fence`) ){
        return 1
    }
    else{
        return 0;
